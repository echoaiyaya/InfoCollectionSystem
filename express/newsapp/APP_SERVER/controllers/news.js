            ...
            .exec((err, newsData) => {
                categories.find({ actived: true })
                    .exec((err, categoriesData) => {
                        news.find()
                            .limit(5)
                            .select("_id title author publicTime")
                            .sort("publicTime")
                            .exec((err, tNews) => {
                                maxPage = Math.ceil(num / 5);
                                currentC = '';
...
const usersNewsByCate = (req, res, next) => {
    cid = req.params.cid;
    news.count({ 'categoryId': cid }, (err, num) => {
        news.find({ 'categoryId': cid })
            .skip((req.params.page - 1) * 5)
            .limit(5)
            .sort('insertTime')
            .populate('categoryId')
                        ...
                        .exec((err, tagsData) => {
                            news.find()
                                ...
                                .exec((err, tNews) => {
                                    ...
                                    newsData.priorities = priorities;
                                    console.log(newsData);
                                    res.render("newsDetail", { article: newsData, tNews: tNews });
                                });
